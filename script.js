// script.js - Modern interaktív funkciók
...
document.querySelectorAll('.add-to-cart-btn').forEach(btn => {
    btn.addEventListener('click', function(e) {
        ...
        let originalHref = this.href; // mentés a hagyományos linkhez

        if (form) {
            // Ha űrlapban van (shoe.php)
            id = form.querySelector('input[name="id"]')?.value;
            size = form.querySelector('select[name="size"]')?.value;
        } else {
            // Egyszerű link (index.php, search.php stb.)
        ...
        // Eredeti ikon elmentése és csere spinnerre
        ...
        .then(data => {
            if (data.success) {
                ...
                // Ha hiba van, visszaállítjuk a gombot, és a felhasználó próbálkozhat újra
            }
        })
        .catch(error => {
            console.error('AJAX hiba:', error);
            showToast('Hálózati hiba, átirányítás...', 'error');
            // Sikertelen AJAX esetén átirányítjuk a hagyományos linkre
