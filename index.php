...
$sort = isset($_GET['sort']) ? $_GET['sort'] : 'price_asc'; // Alapértelmezett: ár szerint növekvő
...
// Összes termék száma a szűrőkkel
...
// Termékek lekérése a szűrőkkel és rendezéssel
...
// Aktuális URL paraméterek megőrzése (kivéve page és category a linkekhez)
...
<section class="hero-section" data-aos="fade-up">
    <div class="row align-items-center">
        <div class="col-lg-6">
            <h1 class="hero-title">
                Lépj stílusosan a <span>világba</span>
            </h1>
            <p class="hero-subtitle">
                Fedezd fel prémium minőségű cipőinket, amelyekkel minden lépés élmény.
                Új kollekciók hetente!
            </p>
        </div>
...
<div class="row mb-4 align-items-end" data-aos="fade-up">
    <div class="col-lg-8">
        <form method="GET" action="index.php" class="row g-2">
            <!-- Kategória megtartása rejtett mezőként -->
...
<div class="category-filters" data-aos="fade-up">
    ...
    <?php foreach ($categories as $cat): ?>
        ...
        $link = '?' . buildQueryString(['category', 'page'], ['category' => $catName]);
        ?>
        <a href="<?= $link ?>" class="category-filter <?= $category === $catName ? 'active' : '' ?>">
        ...
        <?php foreach ($shoes as $index => $shoe): ?>
            <div class="col-md-6 col-lg-4 col-xl-3" data-aos="fade-up" data-aos-delay="<?= $index * 50 ?>">
                <div class="product-card">
                    <?php if ($shoe['is_new'] ?? false): ?>
                        <div class="product-badge">Új</div>
                    <?php endif; ?>
                    
                    <div class="product-image">
                        <img src="<?= htmlspecialchars($shoe['image_url']) ?>" 
                             alt="<?= htmlspecialchars($shoe['name']) ?>">
                        <!-- Hover overlay eltávolítva -->
                    </div>
    ...
    <ul class="pagination justify-content-center">
        ...
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                <a class="page-link" href="?<?= buildQueryString(['page'], ['page' => $i]) ?>">
                    <?= $i ?>
                </a>
            </li>
        <?php endfor; ?>
